...
@property (nonatomic, assign) BOOL mainAlreadyStarted;
...
- (void)didSelectMainTabWithController:(UIViewController *)controller {
    if (!self.mainAlreadyStarted) {
        self.mainAlreadyStarted = YES;
        [self checkTabsController:controller];
        
        MainCoordinator *coordinator = [[MainCoordinator alloc] initWithNavigationController:(UINavigationController *)controller];
        [coordinator start];
        [self addDependency:coordinator];
    }
}
